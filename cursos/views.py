...
from cursos.models import Aulas, Comentarios, Cursos, NotasAulas
import json
...
def aula(request, id):
  if request.session.get('usuario'):
    ...
    return render(request, 'aula.html', {'aula': aula, 'usuario_id': usuario_id, 'comentarios': comentarios, 'request_usuario': request_usuario, 'usuario_avaliou': usuario_avaliou, 'avaliacoes': avaliacoes})
  else:
    return redirect('/auth/login/?status=2')
...
def comentarios(request):
  ...
  comentario_instancia = Comentarios(usuario_id = usuario_id, comentario = comentario, aula_id = aula_id)
...
def processa_avaliacao(request):
  if request.session.get('usuario'):
    ...
    if not usuario_avaliou:
      nota_aulas = NotasAulas(aula_id = aula_id, nota = avaliacao, usuario_id = usuario_id)
      nota_aulas.save()
